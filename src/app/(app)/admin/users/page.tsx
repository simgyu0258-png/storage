...
export default async function AdminUsersPage({
  ...
  const deptPage = Math.max(1, Number(searchParams?.deptPage) || 1);

  const [deptTotal, pagedDepts, allActiveDepts, users] = await Promise.all([
  ...
  const totalDeptPages = Math.ceil(deptTotal / DEPT_PAGE_SIZE);
    ...
    <div className="space-y-4">
      ...
      <section className="rounded-xl border border-slate-200 bg-white p-4">
        <div className="mb-2 flex items-center justify-between">
          <h2 className="font-semibold">부서 목록</h2>
          <span className="text-sm text-slate-500">{deptTotal}개 · {deptPage}/{totalDeptPages} 페이지</span>
        </div>
        ...
        {totalDeptPages > 1 && (
          <div className="mt-3 flex items-center gap-2">
            {deptPage > 1 ? (
              <Link href={`?deptPage=${deptPage - 1}`} className="rounded-md border border-slate-300 px-3 py-1 text-sm hover:bg-slate-50">이전</Link>
            ) : (
              <span className="rounded-md border border-slate-200 px-3 py-1 text-sm text-slate-300">이전</span>
            )}
            {deptPage < totalDeptPages ? (
              <Link href={`?deptPage=${deptPage + 1}`} className="rounded-md border border-slate-300 px-3 py-1 text-sm hover:bg-slate-50">다음</Link>
            ) : (
              <span className="rounded-md border border-slate-200 px-3 py-1 text-sm text-slate-300">다음</span>
            )}
          </div>
        )}
      </section>
